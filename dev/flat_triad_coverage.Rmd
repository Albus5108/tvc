...
# build_triad_from_config
    
```{r development-build_triad_from_config}
# You can prepare the code of the build_triad_from_config() function here
```
  
```{r function-build_triad_from_config}
...
build_triad_from_config <- function(env = "spotify"){
  ...
  if(any(is.na(tabTriads[["playlist"]]))) {
    cli::cli_abort(message = "playlist_id is NA : {tabTriads[['name']][is.na(tabTriads[['playlist']])]}")
  }
...
```{r example-build_triad_from_config}
tabTriads <- build_triad_from_config(env = "spotify")
```
  
```{r tests-build_triad_from_config}
