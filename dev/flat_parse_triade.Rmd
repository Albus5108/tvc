...
clean_song_title <- function(.data){
  .data <- .data %>% 
    ...
    dplyr::mutate(Song = gsub(" - .*Remaster.*$", "", Song),
                  Song = gsub(" \\(.* Remaster\\)$", "", Song),
                  Song = gsub(" \\(feat.*\\)$", "", Song),
                  Song = gsub(" \\(from.*\\)$", "", Song),
                  Song = gsub(" - Radio Mix$", "", Song),
                  Song = gsub(" - Radio Edit$", "", Song),
                  Song = gsub(" - Short Version$", "", Song),
                  Song = gsub(" - [0-9]{4} Remix$", "", Song),
                  ...
                  Song = gsub(" - From .Over The Top. Soundtrack$", "", Song),
...
```{r development-fix_album_date}
# You can prepare the code of the fix_album_date() function here
```
  
```{r function-fix_album_date}
#' Fix Album Dates
#' 
