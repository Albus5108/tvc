...
#' @examples
plot_tvc_histogram <- function(songs, triadType = "1 7") {
  # eras <- tibble::tribble(
  #   ~era, ~begin, ~end,
  ...
  songs %>% 
    dplyr::mutate(
      triad_type = factor(dplyr::if_else(triad == triadType,
                                  triadType,
    ...
    ggplot2::ggplot() +
    ggplot2::aes(Album_Year, fill = triad_type) +
  ...
  #   ) %>% 
  #   dplyr::left_join(w_time_space2, by = dplyr::join_by(ISRC)) %>%
  #   dplyr::mutate(weighted_year_country = dplyr::if_else(is.na(weighted_year_country),
  #                                                        1,
  ...
  #   dplyr::distinct(Song, ISRC, Album_Year, triad_type, .keep_all = TRUE) %>%
  #   ## Stat
  #   dplyr::group_by(Album_Year, triad_type) %>%
  #   dplyr::summarise(
  #     Nb_Songs = dplyr::n(),
  #     Weighted_Nb_Songs = sum(weighted_year_country, na.rm = TRUE)
  #   ) %>%
  #   ggplot2::ggplot() +
  #   ggplot2::aes(Album_Year, Weighted_Nb_Songs, fill = triad_type) + #Weighted_Nb_Songs) +
  #   ggplot2::geom_col() +
  #   ggplot2::annotate(geom = "label", x = c(1986, 2025), y = 5, label = c("1986", "2025")) +
  #   scale_fill_tvc_d(palette = "main", direction = -1) +
  #   ggplot2::theme_bw() +
